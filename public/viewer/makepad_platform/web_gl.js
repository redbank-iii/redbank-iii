...
export class WasmWebGL extends WasmWebBrowser {
  ...
  FromWasmBeginRenderTexture(args) {
    ...
    // depth: attach a per-pass renderbuffer (the depth TODO) so 3D
    // render-to-texture passes get real depth testing
    if (this.depth_renderbuffers === undefined) this.depth_renderbuffers = {};
    var gl_rb =
      this.depth_renderbuffers[args.pass_id] ||
      (this.depth_renderbuffers[args.pass_id] = gl.createRenderbuffer());
    gl.bindRenderbuffer(gl.RENDERBUFFER, gl_rb);
    if (gl_rb._width != args.width || gl_rb._height != args.height) {
      gl_rb._width = args.width;
      gl_rb._height = args.height;
    ...
    } else if (!(args.depth_target && args.depth_target.init_only)) {
      clear_flags |= gl.DEPTH_BUFFER_BIT;
    }
    ...
    clear_depth = args.depth_target ? args.depth_target.clear_depth : 1.0;
